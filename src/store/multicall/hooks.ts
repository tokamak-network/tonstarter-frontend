...
function toCallStateWithContract(
  callResult: CallResult | undefined,
  ...
  if (!valid) return INVALID_CALL_STATE
  if (valid && !blockNumber) return LOADING_CALL_STATE
  if (!contract?.interface || !fragment || !latestBlockNumber) return LOADING_CALL_STATE
  const success = data && data.length > 2
  const syncing = (blockNumber ?? 0) < latestBlockNumber
  let result: Result | undefined = undefined
  if (success && data) {
    try {
      result = contract?.interface.decodeFunctionResult(fragment, data)
    } catch (error) {
      console.debug('Result data parsing failed', fragment, data)
      return {
        valid: true,
...
export function useSingleContractMultipleData(
  ...
  return useMemo(() => {
    return results.map((result) => toCallStateWithContract(result, contract, fragment, latestBlockNumber))
...
export function useSingleCallResult(
  ...
  return useMemo(() => {
    return toCallStateWithContract(result, contract, fragment, latestBlockNumber)
