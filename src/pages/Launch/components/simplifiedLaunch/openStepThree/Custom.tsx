...
const Custom = (props: {step: string; index: number}) => {
  ...
  useEffect(() => {
    checkIsIniailized(vault.vaultType, library, vault, setFieldValue).catch(
      (e) => {
  ...
  //setVaultState
  useEffect(() => {
    returnVaultStatus(values, vault.vaultType, vault, hasToken, setVaultState);
    ...
    const status =
      step === 'Deploy'
        ? values.isTokenDeployed === false ||
          ...
          ? true
          : false
        : vault.isSet === true || !hasToken || values.vaults[7].isSet === false
        ? true
        : false;
    ...
    <Flex
      ...
      <Scrollbars
        ...
        <Flex
          ...
          {vault.claim[0].claimTokenAllocation &&
            vault.claim.map((claim: any, index: number) => {
              return (
                <Flex
                  w="100%"
                  justifyContent={'space-between'}
