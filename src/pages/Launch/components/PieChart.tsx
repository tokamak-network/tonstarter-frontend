...
type PieChartProps = {
  ...
  export const PieChart: FC<PieChartProps> = ({pieData}) => {
    ...
    const colors =  ['#8dd3c7', '#b3de69', '#bebada', '#fb8072', '#80b1d3', '#fdb462', '#ffffb3', '#fccde5', '#d9d9d9', '#bc80bd', '#ccebc5', '#ffed6f']
    ...
    return (
        ...
        tooltip={({datum: data}) => (
          <div
            ...
            <div style={{display: 'flex'}}>
              ...
              <p style={{color: data.color}}>{data.value.toString().slice(0,4)}%</p>
            </div>
        ...
        legends={[
          ...
          symbolSize: 8,
          symbolShape: 'circle',
          data: formattedData.slice(0, Math.floor(formattedData.length/2)).map((cur:any,index:number) => ({
              id: cur.id,
          ...
          symbolSize: 8,
          symbolShape: 'circle',
          data: formattedData.slice(Math.floor(formattedData.length/2)).map((cur:any,index:number) => ({
            id: cur.id, 
            label: `${cur.id} (${cur.value.toString().slice(0,4)}%)`,
            color: cur.color
        }))
