...
const DateRangePicker: React.FC<calendarComponentProps> = ({setDate,startTimeCap, duration}) => {
  ...
  const [maxPublicSaleEndDate, setMaxPublicSaleEndDate] = useState(new Date('2023-05-31T23:59:59.999Z'));
  ...
  const createTime = (onClose: any) => {
    ...
    const ends = moment.unix(startTime + 2 * 86400);
  ...
  useEffect(() => {
    if (duration && startTime) {
      const maxPublicSaleEndTimestamp = (startTime * 1000) + duration * 86400 * 1000;
      const newMaxPublicSaleEndDate = new Date(maxPublicSaleEndTimestamp);
      setMaxPublicSaleEndDate(newMaxPublicSaleEndDate);
    }
  }, [duration, startTime]);
              ...
              <Flex flexDir={'row'}>
                <Flex
                  ...
                  <CustomizedCalendar
                    ...
                    startTime={startTime} startTimeCap={startTimeCap}
                    />
                  <CustomClock
                    setTime={setStartTimeArray}
                    calendarType={'start'}
                    startTime={startTime}
                    endTime={startTime} startTimeCap={startTimeCap}></CustomClock>
                </Flex>
                <Flex flexDir={'column'}>
                  <CustomizedCalendar
                    setValue={setEndTime}
                    maxDate={maxPublicSaleEndDate}
                    startTime={startTime}
                    calendarType={'end'}
                    endTime={endTime} startTimeCap={startTimeCap}
                     />
                  <CustomClock
                    calendarType={'end'}
