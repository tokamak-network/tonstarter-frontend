  ...
  if (Pools) {
    ...
    for (const i in Pools) {
      ...
      const token0 = Tokens?.find(
        (t) =>
          t.tokenAddress.toLowerCase() === pool.token0Address.toLowerCase(),
      );
      const token1 = Tokens?.find(
        (t) =>
          t.tokenAddress.toLowerCase() === pool.token1Address.toLowerCase(),
      );
      if (token0 && token1) {
        ...
        // @ts-ignore
        let sumAmount0 = new CurrencyAmount(Token0, 0);
        //@ts-ignore
        let sumAmount1 = new CurrencyAmount(Token1, 0);
        ...
        const total = sumAmount0_N
          ...
          .plus(sumAmount1_N.multipliedBy(Number(pool.token1Price)));
