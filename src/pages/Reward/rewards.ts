...
import {localDumpRewardData, rewardDB} from 'constants/db';
...
const getPoolData = async (
  library: any,
): Promise<FetchPoolData[] | undefined> => {
  try {
    const rewardReq = localDumpRewardData;
    const poolData: PoolData[] = rewardReq.datas;
    ...
    const poolDatas =
      ...
      Promise.all(
        poolData.map(async (data: PoolData) => {
          ...
          const numRewardPrograms = rewardData?.filter(
            (data: interfaceReward) => {
              return data.poolAddress === poolAddress;
            },
          );
          const token0Symbol = await getTokenFromContract(
            token0Address,
            library,
          );
          const token1Symbol = await getTokenFromContract(
            token1Address,
            library,
          );
...
const getRewardData = async (): Promise<interfaceReward[] | undefined> => {
  const rewardData: interfaceReward[] = rewardDB as any;
